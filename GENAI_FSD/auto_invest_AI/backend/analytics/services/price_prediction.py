...
def _fit_xgboost(feature_df: pd.DataFrame, frequency: str) -> ModelResult:
    ...
    for _ in range(horizon):
        latest_features = _build_feature_frame(close_series).dropna().iloc[-1][FEATURE_COLUMNS]
...
def _fit_lstm(feature_df: pd.DataFrame, frequency: str) -> ModelResult:
    ...
    for _ in range(horizon):
        next_scaled = float(model.predict(seq[np.newaxis, :, :], verbose=0)[0][0])
        next_close = float(
            scaler.inverse_transform([[next_scaled] + [0.0] * (len(FEATURE_COLUMNS) - 1)])[0][0]
        )
        next_date = close_series.index[-1] + step_delta
        close_series.loc[next_date] = next_close

        refreshed = _build_feature_frame(close_series)[FEATURE_COLUMNS].dropna().iloc[-1]
        refreshed_scaled = scaler.transform(
            np.asarray([refreshed.to_numpy(dtype=float)], dtype=float)
        )[0]
